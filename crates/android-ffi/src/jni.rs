...
pub unsafe extern "C" fn Java_com_termfast_app_RustBridge_nativeRemoteTunnelSendNewTerminal(
    ...
    let server_id_str = jstring_to_string(&mut env, &server_id);
    ...
    let server_id_opt = if server_id_str.is_empty() { None } else { Some(server_id_str.as_str()) };
    match crate::remote_terminal::send_new_terminal(&pid, shell_opt, name_opt, server_id_opt) {
        Ok(ct) => vec_to_jbytearray(&mut env, &ct),
        Err(e) => {
...
#[cfg(target_os = "android")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn Java_com_termfast_app_RustBridge_nativeRemoteTunnelSendCloseTerminal(
    mut env: JNIEnv,
    _class: JClass,
    pairing_id: JString,
    terminal_id: jint,
    ...
    let pid = jstring_to_string(&mut env, &pairing_id);
    match crate::remote_terminal::send_close_terminal(&pid, terminal_id as u32) {
        Ok(ct) => vec_to_jbytearray(&mut env, &ct),
        Err(e) => {
            log_to_kotlin("error", &format!("nativeRemoteTunnelSendCloseTerminal: {}", e));
            std::ptr::null_mut()
        }
    }
}
