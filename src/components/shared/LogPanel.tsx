...
import { useLogStore, type LogLevel, type LogCategory } from "@/stores/logStore";
      ...
      >
        ...
        {recentErrors.length > 0 && (
          <div className="flex-1 flex items-center gap-2 overflow-hidden">
            {recentErrors.map((e) => (
              <span key={e.id} className="text-xs text-red-500 truncate" title={e.message}>
                ⚠ {e.message}
              </span>
            ))}
          </div>
      ...
      >
        ...
        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          ...
          <input
            ...
            className="text-xs px-2 py-0.5 rounded border border-gray-300 dark:border-gray-600 bg-transparent w-32"
            value={searchQuery}
        ...
        {displayEntries.length === 0 ? (
          ...
          displayEntries.map((entry) => (
            <div key={entry.id} className="py-0.5">
              ...
              <span className={
                entry.level === "error" ? "text-red-500 font-bold" :
                entry.level === "warn" ? "text-yellow-500" : "text-gray-700 dark:text-gray-300"
              }>
