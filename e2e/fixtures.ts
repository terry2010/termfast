    ...
    ({ servers, config }) => {
      ...
      (window as any).__TAURI_INTERNALS__ = {
        invoke: async (cmd: string, args?: any) => {
          ...
          const entry: { cmd: string; args: any; result: any; error?: string } =
            { cmd, args: args || {}, result: null };
          calls.push(entry);
          try {
            let result: any;
            switch (cmd) {
              ...
              case "ipc_get_daemon_status":
                result = {
                  running: true,
                  server_count: store.servers.length,
                  log_count: 0,
                  version: "0.1.0",
                };
                break;
              ...
              case "ipc_get_server_status": {
                ...
                result = {
                  server_id: sid,
                  status: srv?.current_status || "disconnected",
                  ip: srv?.current_ip || null,
                };
                break;
              }
              ...
              case "ipc_get_proxy_status": {
                ...
                result = {
                  server_id: sid,
                  proxy_enabled: srv?.proxy_running || false,
                  socks5_port: srv?.proxy.socks5_port,
                  http_port: srv?.proxy.http_port,
                };
                break;
              }
              ...
              case "ipc_update_trigger": {
                ...
                const srv = findServer(sid);
                if (srv) {
                  const idx = srv.triggers.findIndex(
                    (t) => t.id === args?.trigger_id,
                  );
                  if (idx >= 0) {
                    srv.triggers[idx] = {
                      ...srv.triggers[idx],
                      ...args,
                      id: srv.triggers[idx].id,
                    };
                  }
                }
                result = { success: true };
              ...
              case "ipc_remove_trigger": {
                const sid = args?.params?.server_id;
                const srv = findServer(sid);
                if (srv) {
                  srv.triggers = srv.triggers.filter(
                    (t) => t.id !== args?.params?.trigger_id,
                  );
                }
                result = null;
                break;
              ...
              case "ipc_detect_firewall":
                result = {
                  firewall_type: "ufw",
                  listening_ports: [22, 80, 443, 3000],
              ...
              case "ipc_update_server": {
                ...
                const srv = findServer(sid);
                if (srv) {
                  if (args?.socks5_port != null)
                    srv.proxy.socks5_port = args.socks5_port;
                  if (args?.http_port != null)
                    srv.proxy.http_port = args.http_port;
                }
                result = { success: true };
                break;
              }
              case "ipc_add_trigger_from_template": {
                ...
                const srv = findServer(sid);
                const tmpl = store.config.trigger_templates.find(
                  (t) => t.id === tmplId,
                );
                if (srv && tmpl) {
                  const newTrig = {
...
export async function getIpcCalls(
  page: Page,
): Promise<{ cmd: string; args: any; result: any; error?: string }[]> {
...
export async function getCallsFor(
  page: Page,
  cmd: string,
): Promise<{ cmd: string; args: any; result: any }[]> {
...
export async function getMockStore(
  page: Page,
): Promise<{ servers: MockServer[]; config: MockConfig }> {
